  ...
  return L.divIcon({
    ...
    html: `
      <div style="position: relative;">
        <div style="
          width: 18px; height: 18px; background: #000000;
...
function DrawControl({
  ...
  const startDraw = () => {
    ...
    new L.Draw.Polygon(map, {
      shapeOptions: { color: '#000000', weight: 2, fillColor: '#000000', fillOpacity: 0.2 },
    }).enable()
  }
  ...
  return (
    <>
      ...
      <div className="absolute top-3 right-3 z-[1000] flex flex-col gap-2">
        <button
          onClick={startDraw}
          className="btn-minimal px-4 py-2 shadow-lg text-sm font-medium transition flex items-center gap-2"
        >
        ...
        <button
          onClick={clearAll}
          className="btn-outline px-4 py-2 shadow-lg text-sm font-medium transition"
        >
          Limpiar
        </button>
        {userPosition && (
      ...
      {info && info.vertices > 0 && (
        <div className="absolute bottom-3 left-3 z-[1000] bg-white border border-black px-4 py-3 text-sm max-w-md">
          <div className="flex items-center gap-4 mb-2">
            <div>
              <p className="text-xs text-gray-600">Superficie</p>
              <p className="font-bold text-black">{info.area.toFixed(2)} m²</p>
            </div>
            <div>
              <p className="text-xs text-gray-600">Perímetro</p>
              <p className="font-bold text-black">{info.perimetro.toFixed(2)} m</p>
            </div>
            <div>
              <p className="text-xs text-gray-600">Vértices</p>
              <p className="font-bold text-black">{info.vertices}</p>
            </div>
          </div>
          {vertices.length > 0 && (
            <div className="border-t border-black pt-2 mt-1 max-h-32 overflow-y-auto">
              <p className="text-xs font-semibold text-black mb-1">Coordenadas de vértices:</p>
              {vertices.map((v) => (
                <p key={v.index} className="text-xs text-gray-600 font-mono">
                  V{v.index + 1}: {formatCoord(v.lat, v.lon)}
                </p>
              ))}
              {resumen.length > 0 && (
                <div className="mt-2 border-t border-black pt-2">
                  <p className="text-xs font-semibold text-black mb-1">Linderos (distancia / rumbo):</p>
                  {resumen.map((r) => (
                    <p key={r.index} className="text-xs text-gray-600 font-mono">
